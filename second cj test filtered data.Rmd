...
amce2 <- cj(dfcj2, choice_indicator ~ expected_pension + gender_equality + living_wage + salary_equality + labor_rights + environmental_impact + faith_morality + firearms, id = ~ survey_id,
            estimate = "amce",
            weights = ~ weights)
...
```{r}
amceusw2gender <- cj(dfcj2, choice_indicator ~ expected_pension + gender_equality + living_wage + salary_equality + labor_rights + environmental_impact + faith_morality + firearms, id = ~ survey_id,
            estimate = "amce",
            weights = ~ weights,
            by = ~ gender)
...
```{r}
amceusw2rep <- cj(dfcj2, choice_indicator ~ expected_pension + gender_equality + living_wage + salary_equality + labor_rights + environmental_impact + faith_morality + firearms, id = ~ survey_id,
            estimate = "amce",
            weights = ~ weights,
            by= ~republican)


amceusw2rep = plot(amceusw2rep, group = "republican", vline = 0, feature_headers = F) +
  scale_colour_manual(na.translate = F, values = c("royalblue", "red3"), labels=c('no', 'yes')) + 
  theme(text = element_text(family = "CMU Serif"))

ggsave(amceusw2rep, path = NULL, 
...
amceusw_byprior2 <- cj(dfcj2, choice_indicator ~ expected_pension + gender_equality + living_wage + salary_equality + labor_rights + environmental_impact + faith_morality + firearms, id = ~ survey_id,
...
amceusw.r.age <- cj(dfcj2, choice_indicator ~ expected_pension + gender_equality + living_wage + salary_equality + labor_rights + environmental_impact + faith_morality + firearms, id = ~ survey_id,
            estimate = "amce",
           # weights = ~w_age_rep,
            by = ~republican)
...
mxl_pref <- logitr(
    ...
    pars    = c(
                ...
                "faith_morality",
                "firearms"),
    randPars = c("gender_equality" = "n", "salary_equality" = "n",

...
dfcj <- read_csv("rds_prod.experiment.420656.stacked(9).csv")
```
...
```{r}
dfus = dfcj %>% 
  filter(florida==0 & california==0)
```

```{r}
mmus <- cj(dfus, choice_indicator ~ expected_pension_num + firearms + fossil_fuels + may_employ_children + racial_diversity + gender_equal_pay, id = ~ survey_id,
            estimate = "mm")

plot(mmus, vline = 0.5) #'*# See what is going on with fossil fuels*
```
...
```{r}
dfcjw = dfcj %>% 
  mutate(weight = case_when(florida == 1 ~ 0.066,
                             california == 1 ~ 0.12,
                             florida == 0 & california == 0 ~ 0.814))

table(dfcjw$weight) # Looks good
```
...
mm <- cj(dfcj, choice_indicator ~ expected_pension_num + firearms + fossil_fuels + may_employ_children + racial_diversity + gender_equal_pay, id = ~ survey_id,
            estimate = "mm")

plot(mm, vline = 0.5) #'*# See what is going on with fossil fuels*
...
mm_byfl <- cj(dfcj, choice_indicator ~ expected_pension_num + firearms + fossil_fuels + may_employ_children + racial_diversity + gender_equal_pay, id = ~ survey_id,
            estimate = "mm", 
            by = ~florida)

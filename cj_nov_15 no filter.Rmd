...
table(dfcj_f$q24_gender_o2_female)
...
## Mind I have to apply weights to show a reasonable general plot
...
dfcj =dfcj %>% 
  mutate(prior = as.factor(q6_taking_into_account_esg_factors_o1_yes))

```

# Weird result. **Check again with weights**

For weights: male/female; dem/rep; age bracket; ethnicity

```{r}
dfcj$prior_num = as.numeric(dfcj$prior)
```


```{r}
cor(dfcj$republican, dfcj$prior_num)
...
mmus_byprior <- cj(dfcj, choice_indicator ~ expected_pension + gender_equality + living_wage + salary_equality + labor_rights + environmental_impact + faith_morality + firearms, id = ~ survey_id,
            estimate = "mm", 
            by = ~prior)

amceus_byprior <- cj(dfcj, choice_indicator ~ expected_pension + gender_equality + living_wage + salary_equality + labor_rights + environmental_impact + faith_morality + firearms, id = ~ survey_id,
            estimate = "amce", 
            by = ~prior)

plot(amceus_byprior, group = "prior", vline = 0) 

plot(mmus_byprior, group = "prior", vline = 0.5) 
